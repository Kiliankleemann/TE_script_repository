...
#-N 1 -X 1000 -p 8  FOR UNIQUE

#---- GENERATED 60 GB SAM FILE!!!!
mkdir SAM_files
cat sample_list.txt | while read sample; do
	bowtie2 -N 1 -k 5000 -X 1000 -p 8 -x ~/Desktop/mm10/mm10 -U fastq_files/fastp_sorted/${sample}.fq.gz  -S SAM_files/${sample}.sam 
done

#Second try 
cat sample_list.txt | while read sample; do
	bowtie2 -N 1 -k 1000 -X 1000 -p 8 -x ~/Desktop/mm10/mm10 -U fastq_files/fastp_sorted/${sample}.fq.gz  -S SAM_files/1000_${sample}.sam 
done
####
...
#Custom sample with 5000 allowed multimapps 
cat sample_list.txt | while read sample; do
	samtools sort SAM_files/1000_DE12NGSUKBR137309_1.sam -o BAM_files/${sample}.final.bam
done
...
mkdir TECount_multi
cat sample_list.txt | while read sample; do
	TECount -b BAM_files/${sample}.final.bam  \
                     --GTF GTF_files_TEtranscript/mm10.refGene.gtf \
                     --TE GTF_files_TEtranscript/GRCm38_GENCODE_rmsk_TE.gtf \
                     --mode multi \
                     --outdir TECount_multi\
                     --project ${sample}
done 
...
#TE local 
echo "Quantification of Reads and TEs from aligned and sorted BAM file"
cat sample_list.txt | while read sample; do
	TElocal --sortByPos -b BAM_files/${sample}.final.bam \
               --GTF GTF_files_TEtranscript/mm10.refGene.gtf \
               --TE GTF_files_TEtranscript/GRCm38_GENCODE_rmsk_TE.gtf \
               --mode multi \
               --outdir TETranscripts_multi
...
#Trying Telescope - only annotates TEs
cat sample_list.txt | while read sample; do
	telescope assign BAM_files/${sample}.final.bam GTF_files_TEtranscript/GRCm38_GENCODE_rmsk_TE.gtf
done
...
cat sample_list.txt | while read sample; do
	...
	--genomeDir /Volumes/OhneTitel/Anne_BMDM_try_August_2023/mm10_index/ \
...
#Running dnaPipeTE 
/opt/dnaPipeTE/docker run -it -v ~/Volumes/OhneTitel:/mnt clemgoub/dnapipete:latest
